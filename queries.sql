...
SELECT title, start_price, image_url, bet_step, owner_id, categories.name as category_name FROM lots RIGHT JOIN categories ON categories.id = category_id JOIN bets ON bets.lot_id = lots.id WHERE finish_date > NOW() ORDER BY lots.id, bets.cost  DESC LIMIT 0, 1;
